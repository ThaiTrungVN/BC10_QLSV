...
function layDuLieuDauVao(isAdd) {
     ...
     */
    //Kiểm tra Validation cho input maSV (nếu thêm thì mới kiểm tra maSV, con Cập nhật thì k cần kiểm tra maSV)
    if (isAdd) {
        isValid &=
    ...
    if (isValid) {
        ...
        return sinhVien;//isValid là true
    }
    return null;//isValid là false
}
...
getEle("btnAdd").addEventListener("click", function (event) {
    ...
    //Kiểm tra => Nếu như thông tin hợp lệ (đã khác null) => Add SV
    if (sinhVien) {
        sinhVien.tinhDTB();
...
function suaSinhVien(maSV) {
    ...
    //Mở lại button btnUpdate khi trước đó đã được cho ẩn bằng style="display: none"
    getEle("btnUpdate").style.display = "inline-block";

    //Dom tới các thẻ input show ra value

    getEle("txtMaSV").value = sinhVien.maSV;
    getEle("txtMaSV").disabled = true;//disabled để   ko cho người khác sửa maSV
 ...
 * Tìm kiếm sinh viên
 */
//txtSearch lúc này là thẻ input chứ ko phải là thẻ button nên cái tham số thứ nhất của hàm addEventListener ko còn là sự kiện click nữa mà nó sẽ là sự kiện keyup (keyup có nghĩa là nó sẽ nhận biết cái sự kiện này từ cái bàn phím mà bạn gõ chữ vào, nếu như bạn gõ gì từ bàn phím vào là nó sẽ hiểu)
